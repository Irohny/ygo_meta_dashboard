...
def parse_meta_deck_category_page(html: str) -> tuple[ParsedMetaDeckListing, ...]:
    ...
    for container in soup.select("#latest-decks .deck_article-card-container"):
        ...
        tournament_name = normalize_whitespace(
            tournament_badge.get_text(" ", strip=True)
        )
        ...
        placement_label, placement_sort_value, placement_group_size = parse_placement(
            stats_match.group("placement")
        )
...
def parse_meta_deck_api_page(payload: str) -> tuple[ParsedMetaDeckListing, ...]:
    ...
    for item in raw_items:
        ...
        tournament_name = (
            normalize_whitespace(_string_value(item.get("tournamentName"))) or None
        )
        placement_text = (
            normalize_whitespace(_string_value(item.get("tournamentPlacement"))) or None
        )
        ...
        if placement_text:
            placement_label, placement_sort_value, placement_group_size = (
                parse_placement(placement_text)
            )

        participants_count = _int_value(item.get("tournamentPlayerCount"))
        player_name = (
            normalize_whitespace(_string_value(item.get("tournamentPlayerName")))
            or None
        )
        relative_age = (
            normalize_whitespace(_string_value(item.get("submit_date"))) or None
        )
        deck_slug, deck_site_id = extract_slug_and_site_id(deck_url)
