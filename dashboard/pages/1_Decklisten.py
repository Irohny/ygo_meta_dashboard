...
def _render_deck_context(selected_deck: dict[str, object]) -> None:
    ...
    detail_col_1.markdown(f"**Turnier:** {_format_text(selected_deck.get('tournament_name'))}")
    detail_col_1.markdown(f"**Datum:** {_format_text(selected_deck.get('tournament_date'))}")
    detail_col_1.markdown(f"**Tier:** {_format_text(selected_deck.get('tier'))}")

    detail_col_2.markdown(f"**Spieler:** {_format_text(selected_deck.get('player_name'))}")
    detail_col_2.markdown(f"**Autor:** {_format_text(selected_deck.get('author_name'))}")
    detail_col_2.markdown(f"**Archetyp:** {_format_text(selected_deck.get('archetype_text'))}")

    deck_url = selected_deck.get("deck_url")
    detail_col_3.markdown(f"**Deckname:** {_format_text(selected_deck.get('deck_name'))}")
    detail_col_3.markdown(f"**Land:** {_format_text(selected_deck.get('country'))}")
    if deck_url:
        detail_col_3.markdown(f"**Deck URL:** [Oeffnen]({deck_url})")
    else:
        detail_col_3.markdown("**Deck URL:** -")
...
def _render_kpi_header(selected_deck: dict[str, object], role_metrics: dict[str, object]) -> None:
    ...
    _render_metric(basis_cols[0], "Platzierung", str(selected_deck["placement"]))
    _render_metric(basis_cols[1], "Platzierungs-Perzentil", _format_percent(selected_deck.get("placement_percentile")))
    _render_metric(basis_cols[2], "Teilnehmer", _format_count(selected_deck.get("participants_count")))
    _render_metric(basis_cols[3], "Main gesamt", _format_count(role_metrics.get("main_card_total")))

    composition_cols = st.columns(3)
    _render_metric(
        composition_cols[0],
        "Main Engine %",
    ...
    _render_metric(
        side_cols[0],
        "Handtraps Side",
        _format_side_profile_value(role_metrics.get("side_handtrap_copies"), role_metrics.get("side_handtrap_share_pct")),
    ...
    _render_metric(price_cols[0], "Cardmarket gesamt", _format_eur(selected_deck.get("cardmarket_deck_price_eur")))
    _render_metric(price_cols[1], "Main EUR", _format_eur(role_metrics.get("main_cardmarket_cost_eur")))
    _render_metric(price_cols[2], "Extra EUR", _format_eur(role_metrics.get("extra_cardmarket_cost_eur")))
    _render_metric(price_cols[3], "Side EUR", _format_eur(role_metrics.get("side_cardmarket_cost_eur")))

    st.caption("Rollenpreise beziehen sich auf Main und Side, weil das Extra Deck bewusst nicht in die Rollenklassifikation einfliesst.")

    role_price_cols = st.columns(4)
    _render_metric(role_price_cols[0], "Engine EUR", _format_eur(role_metrics.get("engine_cardmarket_cost_eur")))
    _render_metric(role_price_cols[1], "Handtraps EUR", _format_eur(role_metrics.get("handtrap_cardmarket_cost_eur")))
    _render_metric(
        role_price_cols[2],
        "Boardbreaker EUR",
...
deck_rows = repository.list_deck_summaries(limit=2000, start_date=start_date, end_date=end_date)
if not deck_rows:
...
deck_name_options = list(deck_rows_by_name.keys())
...
selection_col_1, selection_col_2 = st.columns((1, 2))
...
st.caption(f"Im aktiven Zeitraum sind fuer {selected_deck_name} {len(filtered_deck_rows)} konkrete Listen verfuegbar.")

selected_deck = repository.get_deck_summary_extended(int(selected_deck_id), start_date, end_date)
role_metrics = repository.get_deck_role_metrics(int(selected_deck_id), start_date, end_date)
deck_section_composition = repository.get_deck_section_composition(int(selected_deck_id), start_date, end_date)
deck_vs_group_comparison = repository.get_deck_vs_group_section_comparison(int(selected_deck_id), start_date, end_date)
role_cost_distribution = repository.get_deck_role_cost_distribution(int(selected_deck_id), start_date, end_date)
copy_count_histogram = repository.get_deck_copy_count_histogram(int(selected_deck_id), start_date, end_date)
deck_heatmap_rows = repository.get_deck_section_heatmap_rows(int(selected_deck_id), start_date, end_date)
deck_cards = repository.get_deck_cards_detailed(int(selected_deck_id), start_date, end_date)
...
show_detailed_card_tables = table_control_col_1.toggle("Erweiterte Tabellensicht", value=False)
